...
\begin{center}
\vspace*{2cm}
{\LARGE\bfseries Cross-Task Semantic Contradictions in\\[6pt] Vision-Language Models:\\[6pt] Evaluating and Mitigating the\\[6pt] Existence-Localization Gap}

\vspace{1.5cm}
{\large\textbf{Course Project Final Report (Phase 2)}}\\[6pt]
{\large COMP4020 / COMP5040 -- Natural Language Processing}\\[6pt]
{\large University Course V2025/2026}
...
For each benchmark, a dataset-specific extractor handles format differences (e.g., AMBER uses \textit{``in this image''}, MME appends \textit{``Please answer yes or no''}). The localization prompt is then constructed per model family: natural language for Qwen and \texttt{<ref>} tags for InternVL.
...
\textbf{Implementation}: We handled diverse template alignments. For Qwen, we used natural language instructions. For InternVL, we structured referring expression templates with \texttt{<ref>} tags.\\
...
\begin{enumerate}
\item \textit{``Please provide the bounding box coordinate of the region this sentence describes: \texttt{<ref>}a/an \{object\}\texttt{</ref>}''} (Default)
\item \textit{``Please provide the bounding box coordinate of the region this object describes: \texttt{<ref>}a/an \{object\}\texttt{</ref>}''}
\item \textit{``Please provide the bounding box coordinate of the region for this object if it exists: \texttt{<ref>}a/an \{object\}\texttt{</ref>}''}
\item \textit{``Return the bounding box coordinate of the region where this object appears: \texttt{<ref>}a/an \{object\}\texttt{</ref>}''}
\item \textit{``Please provide all bounding box coordinates of the regions where this object is visible: \texttt{<ref>}a/an \{object\}\texttt{</ref>}''}
\end{enumerate}
